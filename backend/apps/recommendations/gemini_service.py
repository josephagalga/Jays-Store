...
# Create the client using our API key
...
def get_product_context(queryset=None):
    """
    Converts our product database into a text description
...
def build_system_prompt():
    """
    Defines who Gemini is and how it should behave.
...
- If a buyer's budget or request doesn't match any products, say so honestly
...
def build_history(conversation_history):
    """
    Converts our stored conversation history into the
    format the new Gemini SDK expects.
    """
    history = []
...
def chat_with_gemini(user_message, conversation_history=None, buyer=None):
    ...
    along with any recommended product IDs.
    """
    # Add personalisation context if buyer is logged in
    personalisation = ''
    if buyer:
    ...
    # Create the chat with system instruction and history
    ...
    # Extract product IDs from response if any were recommended
    product_ids = []
